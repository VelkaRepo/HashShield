    ...
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            if response.startswith("INFECTED"):
                ...
                threat_name = parts[2] if len(parts) > 2 else (parts[1] if len(parts) > 1 else "Unknown Threat")
                return threat_name, engine_type
    except Exception as e:
...
async def scan_file_hybrid_async(filepath, cache, session, yara_rules, args):
    ...
    threat_name, engine_type = await asyncio.to_thread(scan_file_daemon, filepath)
    if threat_name:
        return filepath, True, threat_name, engine_type

    # 2. YARA (Local Patterns)
    is_malicious_yara, rule_name = scan_file_yara(filepath, yara_rules)
    if is_malicious_yara:
        return filepath, True, rule_name, "YARA"

    # 3. VirusTotal (Cloud)
    if not API_KEY: return filepath, False, "Clean (Daemon/YARA passed, VT disabled)", None
        ...
        for file_path, is_infected, threat_name, engine_type, status in results:
            ...
            if status == "QUARANTINED":
                ...
                if engine_type == "HASH":
                    ...
                    engine_label   = "Shield Engine"
                    dist_data["Hash"] += 1
                elif engine_type == "YARA":
                    severity       = "HIGH"
...
async def main_async_scanner(filepaths, args):
    ...
    results    = []
    if yara_rules: logging.info(f"Loaded YARA rules.")
...
def main():
    print_banner()
    parser = argparse.ArgumentParser(description="HashShield: Hybrid Malware Scanner", formatter_class=argparse.RawTextHelpFormatter)
    ...
    try:
        ...
        for filepath, is_malicious, message, engine_type in results:
            if is_malicious:
                infected_results.append((filepath, message, engine_type))
        ...
        for fp, msg in clean_results_list:
            ...
            if filename in bad_archives:
                infected_results.append((fp, "CONTAINER THREAT: Archive contains malicious files.", None))
            else:
