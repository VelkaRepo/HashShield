...
[📄 View Sample Report](https://htmlpreview.github.io/?https://github.com/VelkaRepo/HashShield/blob/main/img/report-preview.html)
...
```
HashShield/
├── hashshield_daemon.py        # Central EDR daemon — API, socket listener, web dashboard
├── rules.yara                  # YARA heuristic ruleset (92k+ patterns)
├── requirements.txt            # Python dependencies
├── pyproject.toml              # Package config — registers `hashshield` CLI command
│
├── src/
│   ├── agent.py                # Agent entrypoint — scan, report, heartbeat
│   ├── scanner.py              # File scanning logic and result aggregation
│   ├── shield_engine.py        # Detection engine — Hash, YARA, VirusTotal layers
│   ├── main.cvd                # ClamAV-compatible signature database (2.5M+ sigs)
│   ├── hashshield-quarantine/  # Quarantine directory for flagged files
│   └── templates/
│       └── report.html         # Jinja2 template for executive HTML reports
│
├── dist/
│   ├── agent                   # Pre-compiled Linux binary (PyInstaller)
│   ├── agent.exe               # Pre-compiled Windows binary (PyInstaller)
│   ├── dashboard.html          # Standalone dashboard build
│   └── install.bat             # Windows installer script
│
├── img/
│   ├── HashShield-banner.png
│   ├── HashShield.png
│   ├── HashShield.svg
│   ├── dashboard-preview.png   # Dashboard screenshot
│   ├── daemon-preview.png      # Daemon startup screenshot
│   └── report-preview.html     # Sample generated report
│
├── hashshield.db               # SQLite database (auto-created on first run)
├── hunt_kali_threats.py        # Utility: targeted threat hunt for Kali Linux paths
├── inspect_db.py               # Utility: inspect and dump database contents
└── scan_cache.txt              # Local scan deduplication cache
```
